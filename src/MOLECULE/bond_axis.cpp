...
void BondAxis::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    // U_kr = 0.5*kr*(r-r0)^2, F = -dU/dr = -kr*(r-r0)
    ...
    // U_kR = 0.5*kR*d^2 => F = -kR*d (already correct)
    // dU/d(g_i1) torque: needs kR*R factor (already correct)
    ...
    if (eflag || evflag) {
      // U_kr   = 0.5*kr*(r-r0)^2       consistent with F = kr*(r-r0)
      // U_kR   = 0.5*kR*d^2            consistent with F = kR*d
      // U_alpha= kalpha*(1-cos(alpha)) consistent with dU/d(cos a) = -kalpha
      // U_phi  = kphi*(1-cos(phi))     consistent with dU/d(cos p) = -kphi
      ebond = 0.5*kr[type]*(r-r0[type])*(r-r0[type])
            + 0.5*kR[type]*d_mag*d_mag
            + kalpha[type]*(1.0-cos(alpha))
            + kphi[type]*(1.0-cos(phi));
...
double BondAxis::single(int type, double /*rsq*/, int i1, int i2, double &/*fforce*/)
{
  ...
  jquat = bonus[ellipsoid[i2]].quat;
  MathExtra::quat_to_mat(jquat,b1); // Bug fix: was iquat, must be jquat
  ...
  a3_0 = (c13*c22 - c12*c23)/(sqrt(d12*d23)); // Bug fix: removed erroneous minus sign (consistent with compute())
  a3 = roundToPrecision(a3_0, 2);
  
  energy = 0.5*kr[type]*(r-r0[type])*(r-r0[type]) // Bug fix: added r0 offset, consistent with compute()
        + 0.5*kR[type]*d_mag*d_mag
        + kalpha[type]*(1.0-cos(alpha))
        + kphi[type]*(1.0-cos(phi));
