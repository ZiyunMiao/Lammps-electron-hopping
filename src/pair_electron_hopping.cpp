...
//
//   Three-segment inter-chain barrier:
//
//   Segment 1 — Flat core  [0, rho1]:
//     U_perp = 0,  C_perp = 0
//     Charge is free within the monomer.
//
//   Segment 2 — Cubic Hermite transition  (rho1, rho_c]:
//     U_perp = Uc * t^3,   t = (rho - rho1) / w
//     C_perp = 3*Uc*t^2 / (w*rho)
//     where rho1 = rho_c/2,  w = rho_c - rho1 = rho_c/2
//           Uc   = 2*alpha*kBT*w/3  (minimum value ensuring U >= 0)
//
//     Derived from cubic Hermite interpolation with:
//       f(0)=0, f'(0)=0  (joins flat core smoothly)
//       f(1)=Uc, f'(1)=2*alpha*kBT*w  (joins linear tail smoothly)
//     With Uc = 2*alpha*kBT*w/3, the cubic simplifies to f(t) = Uc*t^3.
//     U_perp >= 0 for all rho (no negative energy). C0 and C1 continuous.
//
//   Segment 3 — Linear tail  (rho_c, inf):
//     U_perp = 2*alpha*kBT*(rho - rho_c) + Uc
//     C_perp = 2*alpha*kBT / rho
//
//   All quantities (rho1, w, Uc) are computed from alpha, kBT, rho_c at
//   runtime — no hard-coded values, automatically consistent with parameters.
...
{
    // Derived parameters — all computed from inputs, nothing hard-coded.
    const double rho1 = 0.5 * rho_c;           // flat core boundary
    const double w    = rho_c - rho1;           // = rho_c/2, transition width
    const double A    = 2.0 * alpha * kBT;      // outer slope = dU/drho in linear region
    const double Uc   = A * w / 3.0;            // U at rho_c; ensures U>=0 everywhere

    if (rho <= rho1) {
        // Segment 1: flat core — charge is free inside the monomer.
        U_perp = 0.0;
        C_perp = 0.0;

    } else if (rho <= rho_c) {
        // Segment 2: cubic Hermite transition.
        //
        // U_perp(rho) = Uc * t^3,   t = (rho - rho1) / w
        //
        // Derivation: cubic Hermite with f(0)=0, f'(0)=0, f(1)=Uc, f'(1)=A*w
        // General form: f(t) = (A*w - 2*Uc)*t^3 + (3*Uc - A*w)*t^2
        // With Uc = A*w/3: coefficient of t^2 vanishes, reduces to Uc*t^3.
        //
        // C0 and C1 continuous at both rho1 and rho_c.
        // U_perp >= 0 for all rho (no negative energy).
        double t      = (rho - rho1) / w;       // in (0, 1]
        double t2     = t * t;
        double t3     = t2 * t;
        ...
        // dU/drho = (3*Uc/w) * t^2
        // C_perp  = dU/drho / rho
        C_perp = (3.0 * Uc / w) * t2 / rho;

    } else {
        // Segment 3: linear tail.
        // U_perp = A*(rho - rho_c) + Uc
        // dU/drho = A  →  C_perp = A / rho
        U_perp = A * (rho - rho_c) + Uc;
        C_perp = A / rho;
    }
}
                    ...
                    if (itype!=jtype && setflag[jtype][itype]){
                        ...
                        if (stretch_sq < pow(rcut,2)){
                            ...
                            if(nspecial[j][0]==1){
                                ...
                                // --- Inter-chain rounded-linear term (transverse: a1, a2) ---
                                ...
                                // --- Along-chain (g direction) contribution ---
                                // If charge is between two intrachain monomers (dot_product > 0):
                                //   use harmonic Marcus barrier: U_para = 0.5*k_tilde1*a0^2
                                // If charge has moved laterally away from the chain (dot_product <= 0):
                                //   treat as interchain departure: use same three-segment linear potential
                                //   with |a0| as the radial coordinate — no harmonic restoring force.
                                double U_para, C_para;
                                double abs_a0 = fabs(a0);
                                if (dot_product > 0) {
                                    // Intrachain: harmonic in a0
                                    U_para = 0.5 * k_force_1 * a0 * a0;
                                    C_para = k_force_1;  // dU_para/da0 = C_para * a0
                                } else {
                                    // Interchain departure along g: three-segment linear potential
                                    double U_para_val, C_para_raw;
                                    compute_interchain(abs_a0, alpha_val, rho_c, kb_constant*temp,
                                                       U_para_val, C_para_raw);
                                    U_para = U_para_val;
                                    C_para = C_para_raw;  // dU_para/da0 = C_para * a0 (sign in a0)
                                }

                                energy = U_para + U_perp + offset[itype][jtype];

                                double pij = exp(-energy/(kb_constant*temp)) * (1.0/Z);

                                force_x_pair = -pij*(C_para*a0*gx_center + C_perp*a1*hx_center + C_perp*a2*fx_center);
                                force_y_pair = -pij*(C_para*a0*gy_center + C_perp*a1*hy_center + C_perp*a2*fy_center);
                                force_z_pair = -pij*(C_para*a0*gz_center + C_perp*a1*hz_center + C_perp*a2*fz_center);
                                ...
                                // Torque from along-chain term (g direction): k_force_1 -> C_para
                                torque_gi_x = pij*( -C_para*(a0*(gy_center*delz - gz_center*dely)) );
                                torque_gi_y = pij*( -C_para*(a0*(gz_center*delx - gx_center*delz)) );
                                torque_gi_z = pij*( -C_para*(a0*(gx_center*dely - gy_center*delx)) );
                                ...
                                // alignment term uses a0^2; when dot_product<=0, modulation=0 so it vanishes
                            ...
                            if(nspecial[j][0]==2){
                                ...
                                // --- Along-chain (g direction) contribution ---
                                // Determine which neighbor (left or right) the charge is heading toward,
                                // based on the SIGN of a0, then check whether the charge lies between
                                // center and THAT neighbor (not always "left").
                                double V_neighbor_x, V_neighbor_y, V_neighbor_z;
                                if (a0 >= 0) {
                                    // charge is on the "right" side of center along g: compare to atom_k_right
                                    V_neighbor_x = x[atom_k_right][0] - x[atom_k_center][0];
                                    V_neighbor_y = x[atom_k_right][1] - x[atom_k_center][1];
                                    V_neighbor_z = x[atom_k_right][2] - x[atom_k_center][2];
                                } else {
                                    // charge is on the "left" side of center along g: compare to atom_k_left
                                    V_neighbor_x = x[atom_k_left][0] - x[atom_k_center][0];
                                    V_neighbor_y = x[atom_k_left][1] - x[atom_k_center][1];
                                    V_neighbor_z = x[atom_k_left][2] - x[atom_k_center][2];
                                }
                                double v_neighbor[3] = {V_neighbor_x, V_neighbor_y, V_neighbor_z};
                                domain->minimum_image(v_neighbor);
                                double mag_v_neighbor = sqrt(v_neighbor[0]*v_neighbor[0]
                                                            + v_neighbor[1]*v_neighbor[1]
                                                            + v_neighbor[2]*v_neighbor[2]);
                                double dot_product_2_n2 = (v_neighbor[0]*gx_center
                                                          + v_neighbor[1]*gy_center
                                                          + v_neighbor[2]*gz_center) / mag_v_neighbor;
                                double dot_product_n2 = a0 * dot_product_2_n2;

                                double U_para, C_para;
                                double abs_a0 = fabs(a0);
                                if (dot_product_n2 > 0) {
                                    // Intrachain: harmonic Marcus barrier
                                    U_para = 0.5 * k_force_1 * a0 * a0;
                                    C_para = k_force_1;
                                } else {
                                    // Interchain departure along g: three-segment linear potential
            ...
            if (itype!=jtype && setflag[jtype][itype]){
                ...
                if (stretch_sq < pow(rcut,2)){
                    ...
                    if(nspecial[j][0]==2) {
                        ...
                        // Inter-chain rounded-linear contribution (transverse)
                        double rho_ij = sqrt(a1*a1 + a2*a2);
                        double alpha_val = force_constant_2[itype][jtype];
                        double rho_c = force_constant_3[itype][jtype];
                        double U_perp, C_perp;
                        ...
                        // Along-chain (g direction): harmonic if between neighbors, else three-segment
                        // Compare against whichever neighbor a0 points toward (left or right).
                        double V_neighbor_x_pf, V_neighbor_y_pf, V_neighbor_z_pf;
                        if (a0 >= 0) {
                            V_neighbor_x_pf = x[atom_k_right][0] - x[atom_k_center][0];
                            V_neighbor_y_pf = x[atom_k_right][1] - x[atom_k_center][1];
                            V_neighbor_z_pf = x[atom_k_right][2] - x[atom_k_center][2];
                        } else {
                            V_neighbor_x_pf = x[atom_k_left][0] - x[atom_k_center][0];
                            V_neighbor_y_pf = x[atom_k_left][1] - x[atom_k_center][1];
                            V_neighbor_z_pf = x[atom_k_left][2] - x[atom_k_center][2];
                        }
                        double v_neighbor_pf[3] = {V_neighbor_x_pf, V_neighbor_y_pf, V_neighbor_z_pf};
                        domain->minimum_image(v_neighbor_pf);
                        double mag_v_neighbor_pf = sqrt(v_neighbor_pf[0]*v_neighbor_pf[0]
                                                       + v_neighbor_pf[1]*v_neighbor_pf[1]
                                                       + v_neighbor_pf[2]*v_neighbor_pf[2]);
                        double dp2_pf = (v_neighbor_pf[0]*gx_center
                                        + v_neighbor_pf[1]*gy_center
                                        + v_neighbor_pf[2]*gz_center) / mag_v_neighbor_pf;
                        double dp_pf = a0 * dp2_pf;

                        double U_para_pf;
                        double abs_a0_pf = fabs(a0);
                        if (dp_pf > 0) {
                            U_para_pf = 0.5 * k_force_1 * a0 * a0;
                        } else {
                            double dummy_C;
                            compute_interchain(abs_a0_pf, alpha_val, rho_c, kb_constant*temp, U_para_pf, dummy_C);
                        }

                        double energy = U_para_pf + U_perp + offset[itype][jtype];
