            ...
            if(Z > small_1){      
                for (jj = 0; jj < jnum; jj++){
                    ...
                    if (itype!=jtype && setflag[jtype][itype]){
                        // {
                        //     std::cout << "Step: " << update->ntimestep << " i=" << tag[j] << " itype=" << jtype 
                        //               << " j=" << tag[i] << " jtype=" << itype << " setflag[j][j]=" << setflag[itype][itype] 
                        //               << " setflag[i][j]=" << setflag[jtype][itype] << std::endl;
                        // }
                        ...
                        if (stretch_sq < pow(rcut,2)){
                            ...
                            if(nspecial[j][0]==1){
                                ...
                                domain->minimum_image(v2); // Apply PBC to V2
                                V2_x = v2[0];
                                V2_y = v2[1];
                                V2_z = v2[2];
                                ...
                                // double dot_product_1 = delx * gx_center + dely * gy_center + delz * gz_center;
                                // double dot_product_2 = V2_x * gx_center + V2_y * gy_center + V2_z * gz_center;
                                // double dot_product = dot_product_1 * dot_product_2;
                                ...
                                double V0_x = xtmp - x[j][0];
                                double V0_y = ytmp - x[j][1];
                                double V0_z = ztmp - x[j][2];
                                ...
                                // if (fabs(magnitude_V2) > 5.0) {
                                //     std::cout << "Step: " << update->ntimestep 
                                //               << " i: " << tag[j] << " j: " << tag[i] 
                                //               << " |delx|: " << fabs(delx) 
                                //               << " delx: " << delx << " V0_x: " << V0_x  << " V1_x: " << V1_x << " V3_x: " << V3_x 
                                //               << " dot_product: " << dot_product << " dot_product_2: " << dot_product_2 << " a0: " << a0 
                                //               << " k_force_1: " << k_force_1
                                //               << " mag_V2: " << magnitude_V2 << " V2_x: " << V2_x  << " V2_y: " << V3_y << " V2_z: " << V3_z
                                //               << " distance: " << distance
                                //               << std::endl;
                                // }
                                ...
                                //               << " i: " << tag[j] << " j: " << tag[i] 
                                //               << " |delx|: " << fabs(delx) 
                                //               << " delx: " << delx << " V1_x: " << V1_x << " V2_x: " << V2_x << " V3_x: " << V3_x 
                                //               << " dot_product: " << dot_product << " dot_product_2: " << dot_product_2 << " a0: " << a0 
                                //               << " k_force_1: " << k_force_1
                                //               << std::endl;
                                // }
                                ...
                                energy = 0.5*k_force_1*a0*a0 + 0.5*k_force_2*a1*a1 + 0.5*k_force_3*a2*a2+offset[itype][jtype];
...
double ElectronHopping::partition_function(int i) {
    ...
    int k = atom->map(i);
    // std::cout << "Step: " << update->ntimestep << " input tag i=" << i 
    // << " mapped k=" << k << " type[k]=" << type[k]
    // << " tag[k]=" << tag[k] << std::endl;
    double sum = 0.0;
    n_exp_count = 0;
    ...
    if (setflag[itype][itype]){ 
        for (jj = 0; jj < jnum; jj++){
            ...
            if (itype!=jtype && setflag[jtype][itype]){
                // {
                //     std::cout << "Step: " << update->ntimestep << " i=" << tag[j] << " itype=" << jtype 
                //               << " j=" << tag[k] << " jtype=" << itype << " setflag[j][j]=" << setflag[itype][itype] 
                //               << " setflag[i][j]=" << setflag[jtype][itype] << std::endl;
                // }
                factor_lj = special_lj[sbmask(j)];
                j &= NEIGHMASK;        
                ...
                if (stretch_sq < pow(rcut,2)){
                    ...
                    if(nspecial[j][0]==2) {
                        ...
                        //if (k_force_1 > 0 && k_force_2 > 0){
                            double energy = 0.5*k_force_1*a0*a0 +  0.5*k_force_2*a1*a1 + 0.5*k_force_3*a2*a2+offset[itype][jtype];
