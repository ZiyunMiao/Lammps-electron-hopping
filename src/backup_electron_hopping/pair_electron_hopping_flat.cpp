...
//
//   Flat-core linear inter-chain barrier:
//   U_perp = 0                             if rho <= rho_c  (free core, charge inside monomer)
//          = 2*alpha*kBT * (rho - rho_c)   if rho >  rho_c  (linear tail)
//
//   Physical motivation: charge (polaron) is delocalized within a monomer
//   and experiences no transverse restoring force for rho < rho_c.
//   Outside rho_c the exponential-coupling-derived linear barrier applies.
//
//   NOTE: U_perp is C0-continuous at rho=rho_c (both = 0) but
//         C1-discontinuous: force jumps from 0 to 2*alpha*kBT at rho_c.
//         This is physically motivated and limits the usable timestep.
//
// C_perp = 0                  if rho <= rho_c  (zero transverse force in free core)
//        = 2*alpha*kBT/rho    if rho >  rho_c
// so that grad_ri U_perp = C_perp * (a1*h_hat + a2*f_hat)
...
{
    if (rho <= rho_c) {
        // Flat core: charge is free within the monomer.
        // U_perp = 0, no transverse force.
        // C1-discontinuity at rho=rho_c is intentional (force jumps to 2*alpha*kBT).
        U_perp = 0.0;
        C_perp = 0.0;
    } else {
        // Linear tail: U_perp = 2*alpha*kBT*(rho - rho_c)
        // Continuous with flat core at rho=rho_c (both = 0).
        // grad_ri U_perp = (2*alpha*kBT/rho) * (a1*h + a2*f) = C_perp*(a1*h+a2*f)
