...
//   and U_perp = alpha*kBT * rho^2/rho_c        if rho <= rho_c  (quadratic core)
//              = 2*alpha*kBT * (rho - rho_c/2)  if rho >  rho_c  (linear tail)
//
// C_perp = 2*alpha*kBT/rho_c   if rho <= rho_c
...
{
    if (rho <= rho_c) {
        // Quadratic core: U_perp = alpha*kBT * rho^2/rho_c
        U_perp = alpha * kBT * rho * rho / rho_c;
        C_perp = 2.0 * alpha * kBT / rho_c;
    } else {
        // Linear tail: U_perp = 2*alpha*kBT*(rho - rho_c/2)
        U_perp = 2.0 * alpha * kBT * (rho - 0.5 * rho_c);
