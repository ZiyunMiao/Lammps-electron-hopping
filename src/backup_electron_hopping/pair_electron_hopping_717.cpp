...
int ElectronHopping::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
    ...
    if (n < 0) {
        printf("Rank %d: ERROR: negative n=%d in pack_forward_comm\n", comm->me, n);
        error->all(FLERR, "Invalid n in pack_forward_comm");
    }
    for (i = 0; i < n; i++) {
        j = list[i];
        if (j < 0 || j >= atom->nlocal + atom->nghost) {
            printf("Rank %d: ERROR: invalid atom index j=%d, nlocal=%d, nghost=%d\n", comm->me, j, atom->nlocal, atom->nghost);
            error->all(FLERR, "Invalid atom index in pack_forward_comm");
        }
        printf("Rank %d: atom %d, j=%d, ellipsoid=%d, nspecial=%d\n", comm->me, i, j, ellipsoid[j], nspecial[j][0]);
        if (ellipsoid[j] >= 0 && ellipsoid[j] < atom->nlocal + atom->nghost) {
            buf[m++] = ubuf(nspecial[j][0]).d;
            buf[m++] = ubuf(special[j][0]).d;
            buf[m++] = (nspecial[j][0] >= 2) ? ubuf(special[j][1]).d : 0.0;
    ...
    printf("Rank %d: pack_forward_comm n=%d, m=%d\n", comm->me, n, m);
    if (m != n * 7) {
        printf("Rank %d: ERROR: m=%d does not equal n*7=%d\n", comm->me, m, n * 7);
...
void ElectronHopping::unpack_forward_comm(int n, int first, double *buf)
{
    ...
    if (n < 0) {
        printf("Rank %d: ERROR: negative n=%d in unpack_forward_comm\n", comm->me, n);
        error->all(FLERR, "Invalid n in unpack_forward_comm");
    }
    for (i = first; i < last; i++) {
        if (i < 0 || i >= atom->nlocal + atom->nghost) {
            printf("Rank %d: ERROR: invalid atom index i=%d, nlocal=%d, nghost=%d\n", comm->me, i, atom->nlocal, atom->nghost);
            error->all(FLERR, "Invalid atom index in unpack_forward_comm");
        }
        printf("Rank %d: atom %d, i=%d, ellipsoid=%d, nspecial=%d\n", comm->me, i - first, i, ellipsoid[i], nspecial[i][0]);
        if (ellipsoid[i] >= 0 && ellipsoid[i] < atom->nlocal + atom->nghost) {
            nspecial[i][0] = (int) ubuf(buf[m++]).i;
            special[i][0] = (int) ubuf(buf[m++]).i;
            special[i][1] = (nspecial[i][0] >= 2) ? (int) ubuf(buf[m++]).i : 0;
    ...
    printf("Rank %d: unpack_forward_comm n=%d, m=%d\n", comm->me, n, m);
    if (m != n * 7) {
        printf("Rank %d: ERROR: m=%d does not equal n*7=%d\n", comm->me, m, n * 7);
